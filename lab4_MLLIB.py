...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    return (a[0], (len(a[1]), sum(a[1])/float(len(a[1])))) 
...
movieIDsWithAvgRatingsRDD = movieIDsWithRatingsRDD.map(lambda a: (a[0], (len(a[1]), sum(a[1])/float(len(a[1])))))
...
squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .distinct()).map(lambda x: ( pow((x[1][0] - x[1][1]),2)))
...
print numRatings
print 'Squared Errors: ' ,squaredErrorsRDD.collect()
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD)
                        .distinct()).map(lambda x: ( float(pow((x[1][0] - x[1][1]),2))))
...
testForAvgRDD = testRDD.map(lambda x: (x[0], x[1], x[2].mean))
...
myRatedMovies = [
     (2, random.random.choice(10), random.random.randn(1.0,5.0)) for i in range(10)
